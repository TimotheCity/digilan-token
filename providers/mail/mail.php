...
class DigilanTokenProviderMail extends DigilanTokenSocialProviderDummy
{
  ...
  public function connect()
  {
    ...
    $customized_user_info = DigilanToken::sanitize_custom_portal_inputs($_POST);
    ...
    if ($mail) {
      ...
      $mac = $queries['mac'];
      self::authenticateWithMail($sid, $mac, $customized_user_info);
    } else {
      error_log("Failed to authenticate with mail.");
    }
  }

  private function authenticateWithMail($sid, $mac, $customized_user_info)
  {
    $re = '/^[a-f0-9]{32}$/';
