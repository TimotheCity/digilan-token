...
class DigilanTokenConnection
{
  public static function download_mails_csv($date_start, $date_end)
  {
    ...
    */
    ...
    LEFT JOIN {$wpdb->prefix}digilan_token_users_$version
    ON {$wpdb->prefix}digilan_token_connections_$version.user_id = {$wpdb->prefix}digilan_token_users_$version.id
    ...
    */
    ...
    LEFT JOIN {$wpdb->prefix}digilan_token_users_$version
    ON {$wpdb->prefix}digilan_token_active_sessions_$version.user_id = {$wpdb->prefix}digilan_token_users_$version.id
  ...
  public static function get_connection_count_from_previous_week()
  {
    ...
    if ( $res === null ) {
      ...
      $q = $wpdb->prepare(
        "SELECT count(distinct(fk_session_id)),DATEDIFF(NOW(),viewed_visitor.start),display_name
        FROM viewed_visitor
